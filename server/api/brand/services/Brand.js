...
module.exports = {
  ...
  fetchAll: (params, next, { populate } = {}) => {
    ...
    const hook = strapi.hook[Brand.orm];
    // Generate stages.
    const populateStage = hook.load().generateLookupStage(Brand, { whitelistedPopulate: populate }); // Nested-Population
    const matchStage = hook.load().generateMatchStage(Brand, filters); // Nested relation filter
    const aggregateStages = mergeStages(populateStage, matchStage);

    const result = Brand.aggregate(aggregateStages)
      .skip(filters.start)
      .limit(filters.limit);
